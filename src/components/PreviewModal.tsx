...
export default function PreviewModal({ file, onClose, onDownload }: PreviewModalProps) {
  ...
  const isImage = (mime.startsWith("image/") || ["png", "jpg", "jpeg", "gif", "webp", "svg", "bmp"].includes(ext)) && !file?.isText;
  const isVideo = (mime.startsWith("video/") || ["mp4", "webm", "ogg", "mov", "avi", "mkv"].includes(ext)) && !file?.isText;
  const isAudio = (mime.startsWith("audio/") || ["mp3", "wav", "ogg", "m4a", "aac", "flac"].includes(ext)) && !file?.isText;
  const isPDF = (mime === "application/pdf" || ext === "pdf") && !file?.isText;
  const isOffice = ["doc", "docx", "xls", "xlsx", "ppt", "pptx"].includes(ext) && !file?.isText;
  
  const textExtensions = ["txt", "js", "ts", "tsx", "json", "html", "css", "md", "xml", "csv", "yml", "yaml", "ini", "conf", "sh", "py", "java", "cpp", "c", "h", "cs", "go", "rs", "sql"];
          ...
          <div className="flex-1 overflow-y-auto p-6 bg-slate-50/20 dark:bg-slate-950/40 flex items-center justify-center min-h-[300px]">
            {loadingMedia && (
              <div className="flex flex-col items-center justify-center p-12 gap-3 text-slate-500">
                ...
                <p className="text-xs font-semibold">Generating high-fidelity preview...</p>
              </div>
            )}
            ...
            {!loadingMedia && (
              <>
                ...
                {isPDF && (
                  <div className="w-full h-full min-h-[50vh] flex flex-col items-center gap-4">
                    <iframe
                      ...
                      src={`${mediaUrl || file.downloadUrl}#toolbar=0`}
                      title={file.filename}
                ...
                {isOffice && (
                  <div className="w-full h-full min-h-[50vh] flex flex-col items-center gap-4">
                    <iframe
                      id="preview-office-frame"
                      src={`https://docs.google.com/gview?url=${encodeURIComponent(file.downloadUrl)}&embedded=true`}
                      title={file.filename}
                      className="w-full flex-1 min-h-[48vh] rounded-xl border border-slate-200 dark:border-slate-800 shadow-md bg-white"
                    />
                    <a
                      id="preview-office-external"
                      href={file.downloadUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-1 text-xs text-blue-500 hover:text-blue-600 font-medium transition-colors"
                    >
                      <ExternalLink className="w-3.5 h-3.5" />
                      <span>Download file to view locally</span>
                    </a>
                  </div>
                )}
